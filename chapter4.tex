...
\begin{itemize}
	...
	% Custom tags
	\item The total number of tags created by each person in the group.
	
	\item Tags Weight $TW = \frac{TF}{AF}$
	
	where TF (Tag Fields) = the number of tag fields in a contact profile, AF (All Fields) = the total number of all fields in that contact profile.
	
	\item Tags Effectiveness $TE = \frac{TL}{AL}$
	
	where TL (Tags Lookups) = the number of times a user looks up for a contact using tags, AL (All Lookups) = total contacts lookups.
	
	% Connections
	\item The total number of relationships between contacts created by each person in the group.
	
	\item Relationships Weight $RW = \frac{RF}{AF}$
		
	where RF (Relationship Fields) = the number of relationship fields in a contact profile, AF (All Fields) = the total number of all fields in that contact profile.

        \item Contact Relationship Effectiveness $RE = \frac{RL}{AL}$
	
	where RL (Relationships Lookups) = the number of times a user looks up for a contact using relationships, AL (All Lookups) = total contacts lookups.
